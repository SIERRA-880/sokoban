...
public class BgScene extends Scene {
    ...
    public BgScene(MovingBg bg) {
        ...
        Media sound = new Media(new File("build/resources/main/textures/"+Game.resourcePack+"/Sounds/menus/basshunter-dota-hq.wav").toURI().toString());
    ...
    public BgScene(MovingBg bg, int X, int Y) {
        super(bg, X, Y);
        try {
            Image image = new Image(new FileInputStream("build/resources/main/textures/"+Game.resourcePack+"/Menus/cursor_pointerFlat.png"));  //pass in the image path
            setCursor(new ImageCursor(image));
        }
        catch (FileNotFoundException e) {
            e.printStackTrace();
